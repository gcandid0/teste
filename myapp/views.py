...
def ask_known1_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2')
    else:
        form = PropertyForm()
    
    return render(request, 'ask_known1.html', {'form': form})
    
###############################################################################
    
def ask_known2_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyForm(excluded_properties=excluded_properties)
    
    return render(request, 'ask_known2.html', {'form': form})
                    ...
                    if self.phase == 1:
                        ...
                        if self.index1 != 1 and self.index2 != 1:
                            if self.n == 1: self.looking_for_not_p_in_lc()
                                ...
                                # ALTERADO AQUI PARA error_type7
        ...
        for i in range(len(tabela_vs)): #identificando o índice por T na tabela, variando a cada pressão
            ...
            for k in range(len(tabela_vs[i+1][3])):
                try:
                    ...
                    # ALTERADO AQUI PARA error_type7 (era error_type5)
                    return redirect('error_type7')
                c.append(value)
            b_proximo = c
...
def process_values_view(request):
    ...
    try:
        ...
        # Tentativa de arredondamento. Se houver erro de índice, já captura.
        try:
        ...
        # === CORREÇÃO PARA O CASO DA IMAGEM ===
        # Se as propriedades energéticas forem 0, significa que o cálculo falhou silenciosamente
        # (A classe agua_cls inicia elas com 0 e não conseguiu interpolar)
        if (energia_int == 0 and entalpia_esp == 0 and entropia_esp == 0) or volume_esp == 0:
            return redirect('error_type7')
        
        # Verificação adicional: Se a pressão ou temperatura forem 0 (exceto se o input for realmente 0, 
        # mas combinado com entropia 0 geralmente é erro de tabela)
        ...
        # Validação extra de volumes para garantir
        if fase == 3 and (VolumeL is None or VolumeV is None):
             return redirect('error_type7')
