...
from .forms import TempForm, PropertyForm3, SecondPropertyForm3, ThirdPropertyForm3
...
def ask_known1_view3(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_3')
    else:
        form = PropertyForm3()

    return render(request, 'ask_known1_3.html', {'form': form})

###############################################################################

def ask_known2_view3(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = SecondPropertyForm3(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_3')
    else:
        form = SecondPropertyForm3(excluded_properties=excluded_properties)

    return render(request, 'ask_known2_3.html', {'form': form})

###############################################################################

def ask_known3_view3(request):
    if request.method == 'POST':
        form = TempForm(request.POST)
        if form.is_valid():
            temp_value_input = form.cleaned_data['value_input']
            # Armazena o valor da terceira entrada
            request.session['temp_value_input'] = temp_value_input
            return redirect('ask_known4_3')
    else:
        form = TempForm()

    return render(request, 'ask_known3_3.html', {'form': form})


###############################################################################

def ask_known4_view3(request):

    if request.method == 'POST':
        form = ThirdPropertyForm3(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_3')
    else:
        form = ThirdPropertyForm3()

    return render(request, 'ask_known4_3.html', {'form': form})
...
def process_values_view3(request):
    ...
    try:
        ...
        if estados.lista_estados:
            ...
            fase = h.results[0]
            # Try/Except para arredondamentos com validação
            try:
        ...
        # PROCESSO CALOR (terceira propriedade = 8)
        if third_property_choice == 8:
            calor = third_value_input
            entalpia_esp3 = round((calor + entalpia_esp), 6)
            h = agua_cls(1, 1, 4, pressao, entalpia_esp3)
        else:
            h = agua_cls(1, 1, third_property_choice, pressao, third_value_input)
        ...
        try:
            ...
            volume_v2 = None
        try:
            volume_l2 = h.results[1][3][1]
        except IndexError:
            volume_l2 = None
        if volume_v2 is None or volume_l is None:
            # CORRIGIDO
            return redirect('error_type_3')
        ...
        dados_serializaveis = json.loads(json.dumps(teste3, default=str))
        request.session['dados_processo'] = dados_serializaveis

        return render(request, 'results_3.html', {
            'fase': fase,
